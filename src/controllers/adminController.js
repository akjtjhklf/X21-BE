...
const register = async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ message: "Register Successfully !", data: newAdmin, isSuccess: 1 });
    } catch (error) {
...
const login = async (req, res) => {
    ...
    try {
        ...
        // Tạo JWT token
        const token = jwt.sign({ userId: user._id, role: 'admin' }, process.env.JWT_SECRET, { expiresIn: "24h" });
...
module.exports = { login, register, changePassword, getAdminsList,deleteUserById,changeInfo,getAdminById};
