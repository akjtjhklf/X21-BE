...
const deleteSubject = async (req, res) => {
    try {
        ...
        if (!subject) {
            return res.status(404).json({ message: "Subject not found", isSuccess: 0 });
        }
        await Subjects.findByIdAndDelete(id);
        res.status(200).json({ message: "Delete subject successfully !", isSuccess: 1 });
    } catch (error) {
...
const updateSubject = async (req, res) => {
    try {
        ...
        subject.name = name;
        await subject.save();
        res.status(200).json({ message: "Update subject successfully !", data: subject, isSuccess: 1 });
    } catch (error) {
