...
const register = async (req, res) => {
    const { fullName, email, password, avatar, gender, dob, totalpoint, phone } = req.body;
...
const updateTotalPoint = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ message: 'Failed to update total point', isSuccess: 0 });
    }
};
...
const login = async (req, res) => {
    ...
    try {
        ...
        console.log(token)
        res.json({ message: 'Login successful', token: token, user: user, isSuccess: 1 });
    } catch (error) {
...
const changeInfo = async (req, res) => {
    ...
    try {
        ...
        if (email && email !== user.email) {
            ...
            if (emailExists) {
                return res.status(400).json({ message: 'Email already exists', isSuccess: 0 });
            }
        }
        ...
        res.status(500).json({ message: 'Failed to update user info', isSuccess: 0 });
    }
};
...
module.exports = { login, register, changeInfo, getUsersList, getUsersTopRank, updateTotalPoint, getUserById, deleteUserById };
